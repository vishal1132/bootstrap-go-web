...
func logDependencyError(dependency, name string, err error) error {
	if err != nil {
		zap.L().Error("error pinging dependency", zap.String("dependency", dependency), zap.String("name", name), zap.Error(err))
	}
	return err
}
